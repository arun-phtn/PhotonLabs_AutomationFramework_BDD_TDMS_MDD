...
public class CommonLibrary {
     ...
     * //Runtime.getRuntime().exec(UserConfig.projectLocation + File.separator +
     * "Extensions/Killchromedriver.sh" + " start"); Thread.sleep(1000 * 2);
     * Thread.sleep(1000); //Runtime.getRuntime().exec(adbPath + "/adb" +
     * " start-server"); Thread.sleep(1000);
     * //Runtime.getRuntime().exec(UserConfig.projectLocation +
     * "/drivers/chromedriver"); Thread.sleep(1000);//UserConfig
     * System.out.println("initialising the browser"); DesiredCapabilities
     ...
     * capabilities.setCapability("app", "chrome"); webDriver = new
     * RemoteWebDriver(new URL(UserConfig.chromeDriver_location), capabilities);
    ...
    public static WebElement getElementByProperty(String objectProperty, WebDriver webDriver) {
        ...
        try {
            ...
            if (propertyType.equalsIgnoreCase("CSS")) {
                ...
                element = browserWithElementWait
                        .until(ExpectedConditions.presenceOfElementLocated(By.xpath(objectProperty)));
                // highlightElement(webElement, browser); //to avoid overlapping of elements
                // commented the following line of code -Angeline-03AUG2015
            }
        } catch (Exception e) {
